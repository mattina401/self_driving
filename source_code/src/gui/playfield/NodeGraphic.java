...
public class NodeGraphic extends PlayFieldGraphic {
	...
	private void drawHost(Graphics2D g2) {
		...
		if(node.getName().substring(0,1).equals("p")) {
			Image img1 = Toolkit.getDefaultToolkit().getImage("pedestrian1.png");
			g2.drawImage(img1, scale(loc.getX()-6),scale(loc.getY()-6), scale(15), scale(15), null);	
		} else if (node.getName().substring(0,1).equals("c")){
			Image img1 = Toolkit.getDefaultToolkit().getImage("car2.png");
			g2.drawImage(img1, scale(loc.getX()-6),scale(loc.getY()-13), scale(20), scale(23), null);
		} else if (node.getName().substring(0,1).equals("i")){
			Image img1 = Toolkit.getDefaultToolkit().getImage("intersection.png");
			g2.drawImage(img1, scale(loc.getX()-6),scale(loc.getY()-13), scale(20), scale(23), null);
		}
		else {
			Image img1 = Toolkit.getDefaultToolkit().getImage("bus1.png");
			g2.drawImage(img1, scale(loc.getX()-8),scale(loc.getY()-13), scale(25), scale(25), null);	
		}
		...
		if (drawNodeName) {
			...
			g2.drawString(node.toString(), scale(loc.getX()-13),
					scale(loc.getY()-7));
		}
	}
